...
public class TypeParameterResolverTest {
    /**
     * 关于ParameterizedType中：getRawType.getTypeParameters与getActualTypeArguments之间的对应关系
     */
    @Test
    public void myTest() {
        assert Level2Mapper.class instanceof Class == true;
        Type type = Level2Mapper.class.getGenericInterfaces()[0];
        //type=Level1Mapper<java.util.Date, java.lang.Integer>
        assert type instanceof ParameterizedType;
        Type[] args = ((ParameterizedType) type).getActualTypeArguments();
        //args={Date.class,Integer.class}
        assert ((ParameterizedType) type).getRawType() == Level1Mapper.class;
        TypeVariable<?>[] typeVariables = ((Class) ((ParameterizedType) type).getRawType()).getTypeParameters();
        //typeVariables={E,F}
        //typeVariables与args的关系为E对应Date.class，F对应Integer.class
        //以上结论：
        //ParameterizedType含有两个信息：getActualTypeArguments 参数的实际类型信息{Date.class,Integer.class}、getRawType type实际类型Class
        //而type实际类型Class，的getTypeParameters()又会获取参数类型{E,F}，这样{E,F}就可以与{Date.class,Integer.class}对应上了
    }
    ...
    public void testReturn_Anonymous() throws Exception {
        Calculator<Integer> instance = new Calculator<Integer>();
