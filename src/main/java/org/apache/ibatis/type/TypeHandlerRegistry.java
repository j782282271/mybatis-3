...
public final class TypeHandlerRegistry {
    ...
    private <T> TypeHandler<T> getTypeHandler(Type type, JdbcType jdbcType) {
        ...
        if (jdbcHandlerMap != null) {
            ...
            if (handler == null) {
                // #591
                //如果没找到jdbcType类型的typeHandler，则判断jdbcHandlerMap是否只含有一种TypeHandler，如果只含有一种则返回这一种
    ...
    /**
     * jdbcHandlerMap.values中如果含有类型不同的TypeHandler则返回null，如果所有类型均相同，则返回该类型
     */
    ...
    public <T> void register(TypeHandler<T> typeHandler) {
        ...
        //没找到mappedTypes注解，但是typeHandler继承自TypeReference
        //可以通过TypeReference.getRawType，得到typeHandler父类的参数类型，用于注册
    ...
    /**
     * 使用反射构造typeHandlerClass实例
     */
    @SuppressWarnings("unchecked")
    public <T> TypeHandler<T> getInstance(Class<?> javaTypeClass, Class<?> typeHandlerClass) {
        if (javaTypeClass != null) {
            try {
                //如：EnumTypeHandler就是以Class作为构造函数的参数的
